...
class Analysis:
    def __init__(self, guerzhoy_analysis=True, model_analysis=True):
        ...
        if guerzhoy_analysis:
            self.analysis_board = make_empty_board(15)
        ...
        if model_analysis:
            self.analysis_board_2 = np.zeros((15,15,2,), dtype=np.float32)
            self.model_policy = [[Label(self.root, text="N", image=blank) for __ in range(15)] for _ in range(15)]
            for i in range(15):
                for j in range(15):
                    self.model_policy[i][j].grid(row=i, column=j+10, padx=15, pady=15)
        
        # Creates the GUI for the board
        self.board = [[Label(self.root, image=blank) for __ in range(15)] for _ in range(15)]
        for i in range(15):
            for j in range(15):
    ...
    def reset_boards(self):
        ...
        try:
            ...
            self.model_policy = [[Label(self.root, image=blank) for __ in range(15)] for _ in range(15)]
            for i in range(15):
                for j in range(15):
                    self.model_policy[i][j].grid(row=i, column=j+10, padx=15, pady=15)
        except NameError:
    ...
    def play(self, move, color):
        ...
        """
        i, j = move // 15, move % 15 # not good practice but I'm lazy
        ...
        try:
            ...
            if color=="white":
                policy, value = self.analysis_model(np.flip(self.analysis_board_2, axis=2).reshape(1,15,15,2,), training=False)
                self.model_eval.grid_forget()
                self.model_eval = Label(self.root, text=f"{int(1000*value[0])}m")
                self.model_eval.grid(row=10, column=14)
            else:
                policy, value = self.analysis_model(np.reshape(self.analysis_board_2,(1,15,15,2,)), training=False)
                self.model_eval.grid_forget()
                self.model_eval = Label(self.root, text=f"{int(-1000*value[0])}m")
                self.model_eval.grid(row=10, column=14)
            ...
            for i in range(15):
                for j in range(15):
                    self.model_policy[i][j].grid_forget()
                    self.model_policy[i][j] = Label(self.root, text=f"{100*policy[i*15+j]:.2f}%", bg=(("#0" if int(256*policy[i*15+j]) < 16 else "#") + hex(int(256*policy[i*15+j]))[2:] + "0000"), fg="white", font="Times 28 bold")
                    self.model_policy[i][j].grid(row=i, column=j+10, padx=15, pady=15)
        except NameError:
