...
#
# Board represented with numpy array shape (15,15,2,)
...
def check_legal_actions(board):
    """Return the legal actions like that returned by the policy head in shape (225,)
    """
    return board[:, :, 0] == board[:, :, 1].reshape(225,)
...
def move_on_board(board, move, player=1, takeback=False, safe=True):
    ...
    if not takeback and safe:
        if board[move // 15, move % 15, player - 1] > 0.0:
            raise ValueError(
                f"The move {move} is illegal on {print_board(board)}")
    board[move // 15, move % 15, player - 1] = 0.0 if takeback else 1.0
...
class Game:
    def __init__(self, black=np.zeros((15, 15, 2,), dtype=np.float32), white=np.zeros((15, 15, 2,), dtype=np.float32)):
